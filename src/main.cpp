...
uint256 hashGenesisBlock("0x21c840e396507055e630406bc1a86990a560b28a5385fa9c502e1b857d38d49a");
...
{
    // Poscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    //int64 nSubsidy = ((rand()% 10) + 1) * 10 * COIN;
    ...
    // nSubsidy >>= (nHeight / 840000); // Poscoin no cut in half

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60;     // Poscoin: 1hour
static const int64 nTargetSpacing = 1 * 60;       // Poscoin: 1minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Poscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc56abdf842f254977b559b0480f9dede2c1ba2fd80080eb5b198f6450fb5c3e0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nNonce   = 2085455375;

        if (fTestNet)
        {
            block.nTime    = 1614843130;
            block.nNonce   = 386982714;
        }
	...
	//copy 20200305
        ...
        assert(block.hashMerkleRoot == uint256("0xed378b4cff7414631d19acb53d02b71a60cc3d5a62e942dcf678fde9a92ac983"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Poscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PoscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PoscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PoscoinMiner(CWallet *pwallet)
{
    printf("PoscoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running PoscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PoscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PoscoinMiner, pwallet));
}
